...
internal static partial class UdpManyToManyPortLinkMakerLoggers
{
    ...
    public static partial void LogStartedToTryUdpConnectionWithRemoteIpe(this ILogger logger, int localPort,
        IPEndPoint remoteIpe);

    [LoggerMessage(LogLevel.Error, "[UDP_S2M] {LocalPort} Failed to try UDP connection with {RemoteIpe}")]
    public static partial void LogFailedToTryUdpConnectionWithRemoteIpe(this ILogger logger, Exception ex,
        int localPort, IPEndPoint remoteIpe);
}